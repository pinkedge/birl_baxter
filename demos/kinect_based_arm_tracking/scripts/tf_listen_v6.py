...
prev_data = {'left':[0.0, 0.0, 0.0, 0.0], 'right':[0.0, 0.0, 0.0, 0.0]};


prev_time = {'left':0.0, 'right':0.0}
...
def track_one_arm(shoulder, shoulder_x, elbow, hand, torso, head, base, listener, limb, limb_name):
    ...
    diff_s0 = abs(angles[limb_name+'_s0']-radius_s0)
    diff_s1 = abs(angles[limb_name+'_s1']-radius_s1)
    diff_e0 = abs(angles[limb_name+'_e0']-radius_e0)
    diff_e1 = abs(angles[limb_name+'_e1']-radius_e1)
    
    #print "diff_s0:", diff_s0
    #print "diff_s1:", diff_s1
    #print "diff_e0:", diff_e0
    #print "diff_e1:", diff_e1
    
    
    now_time = time.time()
    
    
    if diff_s0 > threshold_upper or diff_s1 > threshold_upper or diff_e0 > threshold_upper or diff_e1 > threshold_upper:
        flags[limb_name] = True
        
    if diff_s0 < threshold_lower and diff_s1 < threshold_lower and diff_e0 < threshold_lower and diff_e1 < threshold_lower:
        flags[limb_name] = False
    
    
    if flags[limb_name]:
        print "angle mode."
        angles[limb_name+'_s0'] = radius_s0
        angles[limb_name+'_s1'] = radius_s1
        angles[limb_name+'_e0'] = radius_e0
        angles[limb_name+'_e1'] = radius_e1
        limb.set_joint_positions(angles)
    else:
        
        lapse = (now_time-prev_time[limb_name])
        
        print "vel mode."
        #print "lapse:", lapse

        cmd = dict([(joint, 0) for joint in limb.joint_names()])

        now_vel_s0 = (radius_s0-prev_data[limb_name][0])/lapse
        now_vel_s1 = (radius_s1-prev_data[limb_name][1])/lapse
        now_vel_e0 = (radius_e0-prev_data[limb_name][2])/lapse
        now_vel_e1 = (radius_e1-prev_data[limb_name][3])/lapse

        counter[limb_name] = counter[limb_name]+1
        data_set[limb_name].append([now_vel_s0,
                                now_vel_s1,
                                now_vel_e0,
                                now_vel_e1])
        ...
        if counter[limb_name] == times:
            ...
            cmd[limb_name+'_s0'] = command[0]
            cmd[limb_name+'_s1'] = command[1]
            cmd[limb_name+'_e0'] = command[2]
            cmd[limb_name+'_e1'] = command[3]

            print limb_name,'_s0:', cmd[limb_name+'_s0']
            print limb_name,'_s1:', cmd[limb_name+'_s1']
            print limb_name,'_e0:', cmd[limb_name+'_e0']
            print limb_name,'_e1:', cmd[limb_name+'_e1']
    ...
    prev_data[limb_name][0] = radius_s0
    prev_data[limb_name][1] = radius_s1
    prev_data[limb_name][2] = radius_e0
    prev_data[limb_name][3] = radius_e1
    
    
    prev_time[limb_name] = now_time
...
if __name__ == '__main__':
    ...
    prev_time['left'] = time.time()
    prev_time['right'] = time.time()
