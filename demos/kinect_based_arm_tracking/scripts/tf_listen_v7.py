...
def tf_2_angles(shoulder, shoulder_x, elbow, hand,  torso, head, base, listener, limb_name):
    ...
    data.append(b_2_s_x)
    data.append(b_2_e)
    data.append(b_2_hand)
    data.append(b_2_t)
    data.append(b_2_head)

    data_set[limb_name].append(data)
        ...
        for z in range(0, 2):
            ...
            for i in range(0, 6):
                for j in range(0, times):
                    sum[i] = data_set[i][j][z]+sum[i]
            sum_data[z][i] = sum[i]/times

        for i in range(0, 2):
            b_2_s[i] = sum_data[i][0]
            b_2_s_x[i] = sum_data[i][1]
            b_2_e[i] = sum_data[i][2]
            b_2_hand[i] = sum_data[i][3]
            b_2_t[i] = sum_data[i][4]
            b_2_head[i] = sum_data[i][5]

        counter[limb_name] = counter[limb_name]-1
        data_set[limb_name].remove(0)
