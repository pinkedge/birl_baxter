...
class computerApproachPose(object):
    ...
    def step_move_joints(self,current_pose,step):
        next_pose=copy(current_pose)
        _x=copy(current_pose['position'][0])
        _y=copy(current_pose['position'][1])
        _z=copy(current_pose['position'][2])
        _z=_z+step
        next_position=baxter_interface.limb.Limb.Point(_x,_y,_z)
        next_pose['position']=copy(next_position)
        ik_solver=computerIK()
        move_joints=ik_solver.calIK_PY_KDL(self._limb,next_pose)
        return move_joints
